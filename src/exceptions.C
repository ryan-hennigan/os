...
extern "C" void lowlevel_dispatch_exception(REGS * _r) {
  ExceptionManager::dispatch_exception(_r);
}
...
ExceptionHandler * ExceptionManager::handler_table[ExceptionManager::EXCEPTION_TABLE_SIZE];
...
ExceptionManager::ExceptionManager(IDT& _idt) :
   idt(_idt)
{
}


void ExceptionManager::init_dispatcher()
 {
...
void ExceptionManager::dispatch_exception(REGS * _r) {
  ...
  if (!handler) {
    if(exc_no < EXCEPTION_TABLE_SIZE){
        Console::puts(exception_names[exc_no]);
        Console::puts(" Exception\n");
    }
...
void ExceptionManager::register_handler(unsigned int       _isr_code,
...
void ExceptionManager::deregister_handler(unsigned int    _isr_code) {
  assert(_isr_code >= 0 && _isr_code < EXCEPTION_TABLE_SIZE);
